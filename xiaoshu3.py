...
'''
小数点不是 单独一个格，紧贴数字，和数字一起，

数字识别出来了，mask 去掉，剩下就是 点dot

轮廓识别，小数点 面积最小，  length长度，几个数字， 分位线，离的哪个近，就放在哪个位置。


paddleocr识别出数字，轮廓，  分位线

分位线什么时候拿到


类似车牌识别，颜色分割，投影法， 背光，拿到液晶全部roi1 
送给paddleocr 识别 拿到仅仅数字的roi2  length 分位线
然后各种opencv 轮廓，拿到小数点位置，纵坐标，即x坐标，
和分位线比较，两两做差，做小的 
就放在哪个位置
...
def predict_digit( digit_mat):
    ...
    _, results, neigh_resp, dists = k_nearest.findNearest(npaROIResized, k=1)
...
cv2.imwrite("digit_dot.jpg",img)

for c in contours:
    area = cv2.contourArea(c)
    print(area)
    if area < 500:  #2000  框出小数点，0和7  没有8    500，就剩小数点一个了
        cv2.drawContours(img, [c], -1, (36,255,12), -1)

#cv2.imwrite("dot_only.jpg",img)
cv2.imshow('x', img)
cv2.waitKey(0)
...
for contour in contours:
    ...
    if ((size > 2000 and aspect >= desired_aspect - aspect_buffer and aspect <= desired_aspect + aspect_buffer) or
        ...
        cv2.rectangle(img, (x, y), (x + w, y + h), (0, 0, 255), 2)  #bgr 红色

cv2.imwrite("dotposition.jpg",img)
...
'''
Threshold: 15
Average Digit Height and Y: 66.0 and 13.666666666666666
Digit: 0
Digit: 7
Digit: 8
Potential Digits 3
Potential Decimals 2
String: .078
